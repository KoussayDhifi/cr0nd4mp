...
def extract_cron_bsd (log_splitted: list[str]) -> CronJob:
	...
	return CronJob (
		...
		log_type = log_splitted[6]
	) 


def extract_cron_entries (file_path: str="/var/log/syslog", log_entry_types: list[str]=["CMD"]) -> list[CronJob]:
		...
		for log in logs:
			...
			if log_format == "iso" :
				...
				if "CRON" in log_splitted[2].upper():	
					...
					if (job.log_type in log_entry_types):
